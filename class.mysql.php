...
class MySQL
{
    ...
    function MySQL($database, $username, $password, $hostname = 'localhost', $persistant = false)
    {
    ...
    function Insert($vars, $table, $exclude = '')
    {
        ...
        return $this->ExecuteInUpSQL($query);
    }
    ...
    // Executes MySQL insert query

    function ExecuteInUpSQL($query)
    {
        $this->lastQuery = $query;
        if ($this->result = mysql_query($query, $this->databaseLink)) {
            $this->lastInsID = mysql_insert_id($this->databaseLink);
    ...
    {
        return $this->ExecuteSQL("SELECT LAST_INSERT_ID() AS lastId");
    }
    ...
    public function getField($tableName, $field, $id)
    {
        if (!isset($id) || empty($id) || is_null($id)) return '';
